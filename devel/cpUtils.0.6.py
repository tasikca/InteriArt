...
# ver 0.0.6
#
# Version 6 adds the ability to extrude a k-gon along a
...
#
# Version 5 adds a tulip function that rotates a curve
# to create tulip petals surface.
#
# Version 4 adds a plotDaisy function that attaches a central
...
# plotTulip
#
def plotTulip(numFacets,c,numPetalsPerLayer,vertScaling,\
   ...
   for v in range(len(vertScaling)):
      for p in range(len(petalStartStop)-1):
         rAngle = np.linspace(rotPetalLayer[v]+petalStartStop[p]+petalSpace,\
                              rotPetalLayer[v]+petalStartStop[p+1]-petalSpace,30)
...
# daisy orthogonoly.
#
# As of version 0.6 we have a return item
