...
# ver 0.0.2
#
# This is the second version that aligns with the presentation 
# in the paper, i.e. it assumes maximization in the dual form and
# it calculates dx, ds, and dy as explained in the paper. It also
# selects mu values to ensure numerical fidelity to the central path.
#
...
#
# calcMaxPathDeviation
#
# Calculates the maximum deviation of the convex
# combination of two elements on the path from the path.
#
# returns the maximum deviation
#
def calcMaxPathDeviation(s1,y1,mu1,s2,y2,mu2):
   return np.linalg.norm(0.25*np.array(y1+y2)*np.array(s1+s2) - 0.5*(mu1+mu2))
...
#
def divideMuInterval(A,b,c,mu1,mu2,cpDict,errTol):
   ...
   # check for continued division
   # left (larger) interval
   err = calcMaxPathDeviation(cpDict[mu1]['s'],cpDict[mu1]['y'],mu1, \
                              cpDict[muMid]['s'],cpDict[muMid]['y'],muMid)
   if err > errTol:
      divideMuInterval(A,b,c,mu1,muMid,cpDict,errTol)
   # right (smaller) interval
   err = calcMaxPathDeviation(cpDict[muMid]['s'],cpDict[muMid]['y'],muMid, \
                              cpDict[mu2]['s'],cpDict[mu2]['y'],mu2)
   if err > errTol:
      divideMuInterval(A,b,c,muMid,mu2,cpDict,errTol)
   # we should be done
   return 
...
def generatePath(A,b,c):
   errTol = 10**(-4)        # maximum deviation from the central path
   muSmallest = np.exp(-16) # surrogate for mu = zero
   muLargest  = 1000        # surrogate for mu = infty
   m,n = np.shape(A)
   ...
   pathDev = calcMaxPathDeviation(cpDict[muLargest]['s'],cpDict[muLargest]['y'],muLargest, \
                                  cpDict[muSmallest]['s'],cpDict[muSmallest]['y'],muSmallest)
   if pathDev > errTol:
      divideMuInterval(A,b,c,muLargest,muSmallest,cpDict,errTol)
   #
