...
def generateOldPath(A,b,c,tol):
   maxKappa = tol           # maximum curvature estimate
   muSmallest = np.exp(-16) # surrogate for mu = zero
   muLargest  = 10          # surrogate for mu = infty
   m,n = np.shape(A)
   c = np.matrix(c).T # Make sure to have the correct vector form
...
def writeToSVG(A,b,c,tol):
    n = np.shape(A)[0]
    filename = f"svgNewDataSize{n}.svg"
    width, height = 2, 2
    minX, minY = -1, -1
    numPaths = len(c)

    with open(filename, 'w') as f:
        f.write(f'<svg width="{width}in" height="{height}in" viewBox="{minX} {minY} {width} {height}" xmlns="http://www.w3.org/2000/svg">\n')
        f.write(f'<rect width="100%" height="100%" fill="none"/>\n')

        for t in range(numPaths):
           cp = generatePath(A, b, c[t, :], tol)

           start =  f"M {cp[0,0]:.6f} {cp[0,1]:.6f}"
           lines = " ".join([f"L {p[0]:.6f} {p[1]:.6f}" for p in cp[1:]])

           pathData = f"{start} {lines}"

	       # color could be used here with multiple loops to generate a complete laser cut file that does both etching and cutting
           color = f"rgb({0,0,0})"
	       #f.write(f' <path d="{pathData}" stroke="black" stroke-width="1" fill="none" fill-opacity="0.3"/>\n')
           f.write(f' <path d="{pathData}" stroke="black" stroke-width="0.025"  vector-effect="none" fill="none" fill-opacity="0.3"/>\n')
